...
import eu.mcdb.shaded.minecraft.player.VelocityPlayer;
import eu.mcdb.shaded.universal.Server;
import eu.mcdb.shaded.universal.player.UniversalPlayer;
import eu.mcdb.shaded.util.SLF4JWrapper;
...
final class VelocityServer extends Server {
	...
	public List<UniversalPlayer> getOnlinePlayers() {
		...
		for(Player player : handle.getAllPlayers())
			players.add(new UniversalPlayer(player.getUsername(), player.getUniqueId()));
		return players;
	}
