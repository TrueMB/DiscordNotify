...
public class DC_PlayerInfoCommand extends SimpleAddon {
    ...
    public DC_PlayerInfoCommand(DiscordNotifyMain plugin) {
        super("Disnotify PlayerInfo", "disnotify::playerinfo", "TrueMB", "3.0.0", new String[] { "playerinfo", "pi" }); //TODO
    	...
    	if(args.length == 1) {
    		...
    		this.instance.getExecutor().submit(new Runnable() {
				...
				public void run() {
					...
					if(up.isOnline() && !instance.getUniversalServer().isProxy()) {
					...
					long lastplayed = up.isOnline() ? System.currentTimeMillis() : instance.getOfflineInformationManager().getInformationLong(uuid, InformationType.LastConnection);
					...
					String server = up.getServer() != null ? up.getServer() : instance.getOfflineInformationManager().getInformationString(uuid, InformationType.Bungee_Server);
